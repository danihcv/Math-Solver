...
class Expression:
    ...
    def __init__(self, text):
        ...
        self.text = ""
        elements = re.findall(regexElements, text)
        for i in range(len(elements)):
            if elements[i].isalpha() and i > 0 and elements[i - 1].isalnum():
                self.text += "*"
            self.text += elements[i]

        self.elements = re.findall(regexElements, self.text)
